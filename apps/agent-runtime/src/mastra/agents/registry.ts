...
export const AGENT_MANIFEST: Record<AgentId, AgentManifestEntry> = {
  ...
  'coding-agent': {
    ...
    delegatesTo: [],
    note: 'Implements a Task, invoked through delegate_to_code. draft is always deterministic code, never a model call, for any provider - no Mastra Agent object backs this entry, unlike every other row here (docs/ARCHITECTURE.md section 6.5). execute runs one of three providers per run: AURA\'s own built-in agent (agents/mastra-coding-agent.ts - list_files/read_file/write_file only, no shell tool, contained by path checks rather than a container, the main option) or, if asked for, Claude Code or Codex (Docker-sandboxed, authenticated via the developer\'s own CLI login, not a key). Now also asked to write/update unit and integration tests alongside the implementation (agentVersion 2.0.0) - E2E stays QA\'s job.',
    agentVersion: '2.0.0',
    promptVersion: '2.0.0',
  },
  'qa-agent': {
