...
**Status: Frontend built and verified** (real Docker run, exit 0, files on disk, correct host-user ownership). **Backend/NestJS is wired but not yet reliable** - `npx @nestjs/cli new` intermittently hits a known npm/arborist crash (`Cannot read properties of null (reading 'edgesOut')`) during its own `npm install`; caught during verification, not fixed yet. Backend/Spring Boot, Data, AI, and Integration are not implemented at all - `delegate_to_dev` fails clearly rather than silently doing nothing when asked for one of them. Local scaffolding only — no git branch or PR automation. "Security" is not one of these disciplines at all (see below). Full detail: `docs/adr/0001-dev-agent-scaffold-and-template-strategy.md`.
...
flowchart TD
    ...
    G4 --> SBE[Docker: node:22-slim<br/>nest new - not yet reliable]
...
**Status: mixed, honestly.** AURA's own built-in Coding Agent (provider `mastra`) is **built and verified end-to-end** - a real run against a real file, with a real model, produced exactly the requested change and nothing else (see its own bullet below). Claude Code and Codex (providers `anthropic`/`openai`) are built, typechecked, and their surrounding plumbing (auth, credential storage, internal API call, real CLI flags) exercised directly, but the actual CLI execution inside Docker was not run end-to-end in this session - see "What was and was not verified" below. Cursor is out of scope (poor fit for headless invocation). No git branch/PR automation yet, same as Gate 4.

The Dev agent (section 6.4) only ever runs one fixed scaffold command. Gate 5 is a second, separate step that hands an already-scaffolded Task to a coding agent to actually implement it - three interchangeable options behind the same draft/approve/execute flow: Claude Code or Codex (external CLIs, the developer's own tool and account) or AURA's own built-in agent (no external account at all).
...
- **Credentials stay in apps/api; apps/agent-runtime fetches just-in-time.** Each user connects their own Anthropic/OpenAI key on a new self-service page (`apps/web` → account menu → "Profile & connected accounts"), encrypted at rest (AES-256-GCM, `apps/api/src/lib/crypto.ts`, key from `CREDENTIALS_ENCRYPTION_KEY`) in a new `coding_agent_credentials` table (`supabase/migrations/0004_coding_agent_credentials.sql`, same RLS shape as `profiles`). apps/agent-runtime has no Supabase access of its own — giving it one would duplicate a trust boundary apps/api already owns — so it calls a narrow, internal-only route instead: `GET /internal/credentials/:userId/:provider`, gated by a shared `RUNTIME_INTERNAL_TOKEN` bearer check (not the browser's Supabase session auth), mounted before `requireAuth` the same way `/health` is. The decrypted key is held in memory only for the container's env var and is never logged or written to the draft store.
- **Who's asking.** `delegate_to_code execute` reads `agent?.resourceId` in its tool context - confirmed to already equal the Supabase user id (it is `memory.resource`, set by `run-stream.service.ts` on every turn/resume) with no apps/api changes needed. The same field every other delegate tool already reads as `agent?.threadId`'s sibling.
...
- **Sandbox and CLI flags**, verified against each tool's real `--help` output (not assumed): both install fresh via `npm install -g` and run non-interactively inside the same Docker sandbox Gate 4 uses (`lib/docker-exec.ts`, now also accepting a `env` map passed as `-e` args, never shell-interpolated). Claude Code: `claude -p --dangerously-skip-permissions --output-format json`. Codex: `codex exec --sandbox workspace-write --ask-for-approval never --json` - preferred over Codex's own `--dangerously-bypass-approvals-and-sandbox` since Codex offers a tiered sandbox rather than only an all-or-nothing bypass, layered under AURA's own outer Docker sandbox either way.
- **What was and was not verified in this session.** Verified directly: the built-in `mastra` agent, real end-to-end (real model, real file, made exactly the requested one-line change and nothing else); its path-containment logic against eleven edge cases; both CLIs' real flag names via `--help`; the internal endpoint's auth gate (401 on a missing/wrong token, a clear "table not found" error once past auth, not a crash); `agent?.resourceId` resolving correctly per Mastra's own type definitions; full typecheck across all three apps. **Not verified**: actually running Claude Code or Codex end-to-end against a real API key - the session's own safety tooling declined an unattended `claude --dangerously-skip-permissions` invocation ("Create Unsafe Agents"), and CLI installation/execution was stopped at the user's request before Codex could be tried either. The `0004_coding_agent_credentials.sql` migration also has not been applied yet - no `psql`/Supabase CLI/direct DB connection was available in-session; it needs to be run manually (Supabase Dashboard SQL editor, or `supabase db push` once linked). This does not block the `mastra` provider, which needs no credential at all.
...
- **Phase 3** — started early, Frontend only: the Dev agent, Docker-sandboxed scaffold execution, Gate 4 (section 6.4). Not done: Backend/Data/AI/Integration scaffolds, the sandbox runner as a general capability beyond Docker, PR creation, loop guards.
...
9. **Coding agent for actual feature implementation.** **Resolved and built** (2026-09-18, section 6.5): integrate existing coding CLIs (Claude Code, Codex) pointed at the scaffolded directory rather than a bespoke agent with its own file-edit tools. Gate 5 (`delegate_to_code`), per-user API keys, and the internal credential-fetch route are built; actual CLI execution is not yet verified end-to-end (session's own safety tooling declined an unattended `--dangerously-skip-permissions` run - see section 6.5's own verification note). Still open: a companion read-only viewer for the scaffolded project's files (a separable, smaller piece), and Cursor remains explicitly out of scope (poor fit for headless invocation).
