...
9. **Coding agent for actual feature implementation** (2026-09-18, not started): once a Task is scaffolded (section 6.4), something needs to write the feature code itself - a much bigger step than running one fixed scaffold command. Direction favored in discussion: integrate an existing coding CLI (Claude Code, Codex CLI) pointed at the scaffolded directory, rather than building a bespoke agent with its own file read/write tools - reuses a maturer, already-sandboxed tool instead of re-solving code-editing safety from scratch. Needs its own design pass before building: which CLI(s) to support, how each authenticates (API keys/subscription per tool), how it runs inside (or alongside) the Docker sandbox from section 6.4, what "Gate 5" approval looks like for code changes (not just scaffold execution) - a fundamentally different review shape than "approve a plan, then run one command." A companion read-only viewer for the scaffolded project's files (section 6.4's directories) is a smaller, separable piece of this that could land first.
